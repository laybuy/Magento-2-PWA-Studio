 ...
 * Mocks
 */
const getLaybuyConfigQuery = 'getLaybuyConfigQuery';
const setPaymentMethodOnCartMutation = 'setPaymentMethodOnCartMutation';

jest.mock('@apollo/client');

const setPaymentMethodOnCart = jest.fn();
const getLaybuyConfig = jest.fn().mockReturnValue({
    data: {
        storeConfig: {
            payment_laybuy_payable_to: 'Venia Inc. Test',
            payment_laybuy_mailing_address: 'A test address111212'
        }
    }
});
...
beforeAll(() => {
    useQuery.mockImplementation(query => {
        if (query === getLaybuyConfigQuery) {
            return getLaybuyConfig();
        } else {
            return [jest.fn(), {}];
        }
    });
...
const Component = props => {
    const talonProps = useLaybuy(props);
